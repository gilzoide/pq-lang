...
static inline pq_value *_pq_call_native_function(pq_context *ctx, pq_function_metadata *func_md, int argc, pq_value **argv) {
    ...
    pq_value *return_value = pq_new_uninitialized_value(ctx, pq_type_get_return_type(func_md->signature));
...
static inline pq_value *_pq_arguments_match_signature(pq_context *ctx, pq_type *signature, int argc, pq_value **argv) {
    if(signature) {
        ...
        for(i = 0; i < expected_argnum; i++) {
            pq_type *expected = sig_types[i], *found = argv[i]->type;
            if(pq_type_get_match(expected, found) == PQ_TYPE_MATCH_NONE) {
                return pq_value_ferror(ctx, "Invalid argument %d type: expected %s, found %s", i, expected->name, found->name);
            }
        }
    }
    return NULL;
...
pq_value *pq_call(pq_context *ctx, pq_value *func, int argc, pq_value **argv) {
    if(!func) {
        ...
        if(evaluation_error = _pq_arguments_match_signature(ctx, func_md->signature, argc, argv)) return evaluation_error;
